    ...
    def run(self):
        ...
        for issue in issues:
            ...
            if 'bug' in labels:
                ...
                continue  # Skip issues that don’t fit either category
            data.append({'date': created_date, 'category': category})
        ...
        monthly_trend = df.groupby([pd.Grouper(freq='M'), 'category']).size().unstack(fill_value=0)

        # Plot the trend
        monthly_trend.plot(kind='line', figsize=(12, 6), title="Feature Requests vs. Bug Fix Trends Over Time")
        plt.xlabel("Date")
